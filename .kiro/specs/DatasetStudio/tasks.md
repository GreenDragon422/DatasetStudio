...
- [x] 24. Implement TagDictionaryService
  ...
  - Progress note (2026-03-28): Tag statistics now use both an in-memory per-project cache and a persisted fingerprinted snapshot in the project configuration. `TagDictionaryService` invalidates the live cache on `TagDictionaryChangedMessage`, `TagsChangedMessage`, `AiTaggingCompletedMessage`, `ImageDeletedMessage`, and watcher-originated `TagFilesChangedMessage` events, then recomputes and re-persists frequencies only when the tag-file fingerprint changes.
...
- [x]* 25. Write TagDictionaryService tests
  ...
  - Progress note (2026-03-28): Added regression coverage for persisted tag-stat snapshots and runtime cache invalidation after manual tag edits (`TagsChangedMessage`), auto-tag completion (`AiTaggingCompletedMessage`), image deletion (`ImageDeletedMessage`), and filesystem watcher `.txt` change notifications (`TagFilesChangedMessage`).
...
- [x] 40. Wire AI tagger background processing
  ...
  - [x] 40.5 Add AI model install UX — show install state and an explicit `Download Model` action beside the selector in ProjectOverview and ProjectConfiguration for HF-backed models
  ...
  - Progress note (2026-03-28): Automatic HF model installation was removed from `AiTaggerService.GenerateTagsAsync`. Hugging Face-backed models now require the explicit `Download Model` button beside the selector, that button is disabled once the model is installed, and the shared status copy now makes the install requirement visible in both Project Overview and Project Configuration. Coverage was expanded with `AiTaggerServiceTests` plus a Project Overview regression test for explicit download state, and the full solution verifies cleanly with `dotnet test DatasetStudio.sln --no-restore`.
  - Progress note (2026-03-28): The AI model header chrome now uses a shared `AiModelSummary` control so model name and install state always render together. Project Configuration remains the only screen that exposes the model dropdown; Project Overview now shows the configured model as read-only summary plus the existing explicit download action when the selected model is not installed.
  - Progress note (2026-03-28): Added `fancyfeast/joytag` to the bundled `ai_models.json` catalog as `JoyTag`, based on the published Hugging Face repo. The repo advertises `image-classification` with `model.onnx`, `model.safetensors`, `config.json`, and `top_tags.txt`, so it can now be selected and downloaded through the existing explicit HF CLI download flow without changing the runtime contract.
  - Progress note (2026-03-28): Promoted the preferred SmilingWolf taggers in the bundled catalog so they are easy to find in the selector: `SmilingWolf/wd-eva02-large-tagger-v3`, `SmilingWolf/wd-swinv2-tagger-v3`, and `SmilingWolf/wd-convnext-tagger-v3` now appear near the top with explicit display names.
  - Progress note (2026-03-28): The placeholder AI tagger was replaced with a real local ONNX Runtime GPU path for WD-style taggers. DatasetStudio now keeps one long-lived `InferenceSession` in `TaggerSession`, batches queued images through a channel-backed worker, preprocesses with ImageSharp based on `session.InputMetadata`, loads `selected_tags.csv` once per active model, stores structured rating/general/character tags internally, and writes `.txt` sidecars through `TagSidecarService` as sidecar persistence. Downloaded WD models now request only `model.onnx` and `selected_tags.csv` from Hugging Face, and coverage was expanded with `TagPostProcessorTests`, `ImagePreprocessorTests`, and stronger `AiTaggerServiceTests`. The full solution verifies cleanly with `dotnet build DatasetStudio.sln --no-restore -warnaserror` and `dotnet test DatasetStudio.sln --no-restore`.
  - Progress note (2026-03-28): Expanded the bundled `ai_models.json` catalog to match the TagGUI families you asked for: the remaining TagGUI WD `tagger-v3` models (`SmilingWolf/wd-vit-large-tagger-v3`, `SmilingWolf/wd-vit-tagger-v3`) were added with the same minimal ONNX payload, and the JoyCaption plus Florence-2 entries now use explicit display names so they are easy to spot in the selector. Runtime compatibility is unchanged: the current local ONNX tagging path is still WD-style only.
  - Progress note (2026-03-28): Architectural clarification: tags are first-class entities across the whole app, not image-scoped values. The ONNX tagging flow should be read as producing image-specific tag assignments that reference standalone tags, with flat `.txt` sidecars remaining only a derived sidecar format.
