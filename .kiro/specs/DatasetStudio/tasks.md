...
- [x] 40. Wire AI tagger background processing
  ...
  - Progress note (2026-03-28): The placeholder AI tagger was replaced with a real local ONNX Runtime GPU path for WD-style taggers. DatasetStudio now keeps one long-lived `InferenceSession` in `TaggerSession`, batches queued images through a channel-backed worker, preprocesses with ImageSharp based on `session.InputMetadata`, loads `selected_tags.csv` once per active model, stores structured rating/general/character tags internally, and writes `.txt` sidecars through `TagExportService` as a derived export. Downloaded WD models now request only `model.onnx` and `selected_tags.csv` from Hugging Face, and coverage was expanded with `TagPostProcessorTests`, `ImagePreprocessorTests`, and stronger `AiTaggerServiceTests`. The full solution verifies cleanly with `dotnet build DatasetStudio.sln --no-restore -warnaserror` and `dotnet test DatasetStudio.sln --no-restore`.
